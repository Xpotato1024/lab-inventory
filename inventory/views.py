...
from .placement_order import move_unit_relative
...
def unit_move(request: HttpRequest, code: str) -> HttpResponse:
    ...
    initial = {
        "target_type": MoveUnitForm.TargetType.ZONE,
        "position": MoveUnitForm.Position.RIGHTMOST,
    }
    try:
    ...
    if request.method == "POST" and form.is_valid():
        try:
            _, change = move_unit_relative(
                unit=unit,
                zone=form.cleaned_data["zone"],
                support_unit=form.cleaned_data["support_unit"],
                position=form.cleaned_data["position"],
                reference_unit=form.cleaned_data["reference_unit"],
                note=form.cleaned_data["note"],
                actor=request.user,
            )
